<template>
  <div class="container p-2">
    <h1 class="mb-8 text-lg font-bold">Digest-it Calculator</h1>
    ...
    <ul class="flex border-b mb-4">
      ...
      <li :class="{ '-mb-px': tab == 'circular' }" class="mr-1">
        <a
          :class="{ 'border-l border-t border-r rounded-t': tab == 'circular' }"
          class="bg-white inline-block py-2 px-4 text-blue-500 hover:text-blue-800 font-semibold"
          href="#"
          @click="setTab('circular')"
          >Circular</a
        >
      </li>
      <li :class="{ '-mb-px': tab == 'herd' }" class="mr-1">
        <a
          :class="{ 'border-l border-t border-r rounded-t': tab == 'herd' }"
          class="bg-white inline-block py-2 px-4 text-blue-500 hover:text-blue-800 font-semibold"
          href="#"
          @click="setTab('herd')"
          >Herd</a
    ...
    <div v-if="tab != 'herd'">
      <div class="flex space-x-4 w-full mb-4">
        <div v-if="tab == 'standard'">
          ...
          <input
            ...
            name="length"
            v-model="length"
            class="bg-gray-200 appearance-none border-2 border-gray-200 rounded p-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500 w-24"
          />
        </div>
        <div v-if="tab == 'standard'">
          <label
      ...
      <div class="w-full mb-4">
        ...
        <div class="flex space-x-4">
          <div>
            ...
            <label class="text-sm" for="ft">Feet</label>
          </div>

          <div>
            <input type="radio" value="m" name="unit" v-model="unit" />
            <label class="text-sm" for="m">Meters</label>
          </div>
        </div>
      </div>

      <h2 class="block text-gray-500 font-bold mb-1 pr-4">
        Slurry volume
      </h2>
      ...
      <h2 class="block text-gray-500 font-bold mb-1 pr-4 mt-8">
        Digest-it required
      </h2>
      ...
      if (this.tab == 'standard') {
        if (this.unit == 'm') {
          ...
          return (
            this.length *
            this.ft_to_m *
            (this.width * this.ft_to_m) *
            (this.height * this.ft_to_m) *
            219.969
          )
        }
